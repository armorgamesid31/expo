...
export function InventoryPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(err?.message || 'Envanter alinamadi.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-4 space-y-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground">Stok create/read + hareket guncelleme aktif.</p>
      </div>

      {error ? <p className="text-sm text-red-500">{error}</p> : null}

      <form className="grid grid-cols-2 gap-2 rounded-lg border border-border p-3" onSubmit={createItem}>
        <input className="col-span-2 rounded-md border border-border px-3 py-2 text-sm" placeholder="Urun adi" value={form.name} onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))} />
        ...
        <input className="rounded-md border border-border px-3 py-2 text-sm" type="number" min={0} placeholder="Baslangic stok" value={form.currentStock} onChange={(e) => setForm((prev) => ({ ...prev, currentStock: Number(e.target.value) }))} />
        ...
        <button type="submit" disabled={saving} className="col-span-2 rounded-md bg-[var(--rose-gold)] px-4 py-2 text-sm text-white disabled:opacity-60">{saving ? 'Ekleniyor...' : 'Urun Ekle'}</button>
      </form>
        ...
        {items.map((item) => (
          <div key={item.id} className="rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-xs ${item.lowStock ? 'text-red-500' : 'text-green-600'}`}>{item.lowStock ? 'Dusuk stok' : 'Normal'}</span>
            </div>
      ...
      {!loading && !items.length ? (
        <div className="rounded-lg border border-dashed border-border p-4 text-center text-sm text-muted-foreground">Envanter bos.</div>
      ) : null}
    </div>
  );
}
