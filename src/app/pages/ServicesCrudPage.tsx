...
export function ServicesCrudPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(err?.message || 'Hizmetler alinamadi.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const createItem = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(err?.message || 'Hizmet olusturulamadi.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-4 space-y-4">
      <div>
        <h1 className="text-xl font-semibold">Hizmet Yonetimi</h1>
      ...
      <form className="grid grid-cols-2 gap-2 rounded-lg border border-border p-3" onSubmit={createItem}>
        <input className="col-span-2 rounded-md border border-border px-3 py-2 text-sm" placeholder="Hizmet adi" value={form.name} onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))} />
        ...
        <input className="rounded-md border border-border px-3 py-2 text-sm" type="number" min={5} step={5} placeholder="Sure" value={form.duration} onChange={(e) => setForm((prev) => ({ ...prev, duration: Number(e.target.value) }))} />
